...
class MemberAction extends BaseAction
{
	...
	public function index()
	{
		...
		$dataList = $this->dao->Table(C('DB_PREFIX').'member a')->Join(C('DB_PREFIX').'group b on a.group_id=b.id')->Field('a.*,b.role_name as role_name')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->findAll();
		$page = $p->show();
		if($dataList!=false){
	...
	public function insert(){
		parent::_checkPermission();
		$roleList = M('Group')->Order("id DESC")->findall();
	...
	{
		parent::_checkPermission('Member_insert');
	...
	public function modify()
	{
		...
		$roleList = M('Group')->Order("id")->findAll();
