...
class NewsAction extends BaseAction
{
    ...
    function _initialize()
    {
        parent::_initialize();
        //取所有分类，过滤出新闻模块主ID
        $category = D('Category')->where('module="News" or module="CollegeNews" or module="Link"')->Order('display_order DESC,id DESC')->findAll();
    ...
    public function index()
    {
        ...
        $condition['b.module']=array('in','News,CollegeNews,Link');
        ...
        $dataList = $this->dao->Table(C('DB_PREFIX').'news a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->where($condition)->Field('a.*,b.title as category')->Order($setOrder)->Limit($p->firstRow.','.$p->listRows)->findAll();
        $page = $p->show();
        if ($dataList !== false)
        {
