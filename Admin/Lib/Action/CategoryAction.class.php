...
class CategoryAction extends BaseAction
{
    ...
    function _initialize()
    {
        ...
        $this->assign($data);
        $this->dao = D('Category');
        $this->category = $this->dao->where('module != "Page"')->Order('id DESC')->findAll();
    ...
    public function doInsert()
    {
        ...
        {
            //清空不是根目录的所属模块
            $condition['id']=$_POST['parent_id'];
            $category_id=M('Category')->where($condition)->field('module')->select();
            $module=$category_id[0]['module'];
