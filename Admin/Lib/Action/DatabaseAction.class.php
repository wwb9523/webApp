...
class DatabaseAction extends BaseAction{
    ...
    public function query(){
        parent::_checkPermission();
        $data['uploadTips'] = ini_get('file_uploads') ? '目前系统允许上传:'.ini_get('upload_max_filesize').' 请不要上传超过此范围文件。推荐2M以内，以免上传过程中因浏览器卡死而造成上传失败': 'PHP环境配置为：禁止上传';
        $this->assign($data);
        $this->display();
    }
    ...
    public function export()
    {
       ...
       // dump($tables);
        ...
        $fileName = date('Ymd_Hi').'_'.rand_string(10);
