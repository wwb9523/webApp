...
class JournalAction extends BaseAction
{
    ...
    public function index()
    {
        ...
        $dataList = $this->dao->Table(C('DB_PREFIX').'journal a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->Field('a.*,b.title as category')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->findAll();
        $page = $p->show();
        if ($list !== false)
        {
    ...
    {
        parent::_checkPermission('Journal_insert');
    ...
    {
        parent::_checkPermission('Journal_modify');
        parent::_setMethod('post');
    ...
    {
        parent::_checkPermission('Journal_command');
        if(getMethod() == 'get'){
