...
class RecruitAction extends BaseAction
{
    ...
    public function index()
    {
        ...
        $count=$this->dao->Table(C('DB_PREFIX').'recruit a')->Join(C('DB_PREFIX').'company b on a.company_id=b.id')->where($condition)->count();
        $listRows = empty($pageSize) || $pageSize > 100 ? 15 : $pageSize ;
        $p = new page($count, $listRows);
        $dataList = $this->dao->Table(C('DB_PREFIX').'recruit a')->Join(C('DB_PREFIX').'company b on a.company_id=b.id')->where($condition)->
        order('id DESC')->Field('a.*,b.name as companyName')->Limit($p->firstRow.','.$p->listRows)->findAll();
        $page = $p->show();
        if ($dataList !== false)
        {
    ...
    {
        parent::_checkPermission();
        $companies=M('Company')->field('id,name')->select();
        $domain=M('Domain')->select();
        $education=M('education')->select();
        $computer=M('Level')->where('type="computer"')->select();
        $english=M('Level')->where('type="English"')->select();
        $form=M('work_form')->select();
    ...
    {
        parent::_checkPermission('Recruit_insert');
    ...
    public function modify()
    {
        ...
        $form=M('work_form')->select();
        $condition['a.id'] =$item;
        $record = $this->dao->Table(C('DB_PREFIX').'recruit a')->Join(C('DB_PREFIX').'company b on a.company_id=b.id')
            ->where($condition)->Field('a.*,b.name as companyName')->find();
    ...
    {
        parent::_checkPermission('Recruit_modify');
        parent::_setMethod('post');
