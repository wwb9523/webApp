...
class UserAction extends BaseAction
{
	...
	public function index()
	{
		...
		$dataList = $this->dao->Table(C('DB_PREFIX').'user a')->Join(C('DB_PREFIX').'role b on a.role_id=b.id')->Field('a.*,b.role_name as role_name')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->findAll();
		$page = $p->show();
		if($dataList!=false){
	...
	public function insert(){
		parent::_checkPermission();
		$roleList = M('Role')->Order("id DESC")->findall();
	...
	public function modify()
	{
		...
		$roleList = M('AdminRole')->Order("id")->findAll();
